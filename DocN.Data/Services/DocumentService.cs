...
public class DocumentService : IDocumentService
{
    ...
    {
        // OTTIMIZZAZIONE: AsNoTracking per query read-only di verifica permessi
        var document = await _context.Documents
            .AsNoTracking()
            .Include(d => d.Shares)
    ...
    public async Task<DocumentShareInfo?> GetDocumentSharesAsync(int documentId, string userId)
    {
        ...
        // OTTIMIZZAZIONE: AsNoTracking per query read-only con projection
    ...
    public async Task<int> DeleteAllUserDocumentsAsync(string userId)
    {
        try
        {
            if (string.IsNullOrEmpty(userId))
            {
                _logger?.LogWarning("Attempted to delete all documents with empty userId");
                return 0;
            }
            ...
            {
                _logger?.LogInformation("User {UserId} has no documents to delete", userId);
                return 0;
            }
